...
public class PokerOddsCalculator {
	...
	public static void main(String[] args) {
		...
		int deadCardsNum = (int) (Math.random()*11);
		Card[] deadCards = new Card[deadCardsNum];
		for(int i = 0; i < deadCardsNum; i++) {
			int removeindex = (int)(Math.random()*deck.length);
			deadCards[i] = deck[removeindex];
			deck = removeCard(deck,removeindex);
		}
		System.out.println("Dead Cards");
		if(deadCardsNum>=1) {
			for(int i = 0; i < deadCardsNum;i++) {
				deadCards[i].print();
			}
		...
		//making the board
		int boardsize = (int)(0);//board size from 0 to 4
		Card[] board = new Card[boardsize];
		for(int i = 0; i <boardsize; i++) {
		...
		System.out.println("Player 1 win: " + numerator1/denominator);
		System.out.println("Player 2 win: " + numerator2/denominator);
